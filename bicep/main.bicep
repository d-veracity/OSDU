...
@description('Optional: Server Configuration Overrides - {system}-->(1x2 Burstable) {zone}-->(1x2 Burstable) {user}-->(1x2 Burstable) - Optimized for quota constraints')
param serverConfiguration object = {
  systemPool: 'Standard_B2s' 
  userPool: 'Standard_B2s'
}
...
var rg_unique_id = '${replace(configuration.name, '-', '')}${uniqueString(resourceGroup().id, configuration.name, location)}'
var dnsName = uniqueString(resourceGroup().id, configuration.name, location)
...
module insights 'br/public:avm/res/insights/component:0.6.0' = {
  ...
  params: {
    name: '${replace(configuration.name, '-', '')}${uniqueString(resourceGroup().id, configuration.name, location)}'
    location: location
    enableTelemetry: enableTelemetry
...
module clusterBlade 'modules/blade_cluster.bicep' = {
  ...
  params: {
    ...
    nodeResourceGroupLockDown: clusterConfiguration.enableLockDown == 'true' ? true : false
    ...
    vmSizeSystemPool: serverConfiguration.systemPool == '' ? 'Standard_B2s' : serverConfiguration.systemPool
    vmSizeUserPool: serverConfiguration.userPool == '' ? 'Standard_B2s' : serverConfiguration.userPool
  }
  dependsOn: [
    stampIdentity
    logAnalytics
...
var name = '${replace(configuration.name, '-', '')}${uniqueString(resourceGroup().id, configuration.name, location)}'
...
var vaultSecrets = [
  ...
  }
  {
    ...
    secretValue: substring(uniqueString('dbuser', resourceGroup().id, configuration.name, location), 0, 8)
  }
  {
  ...
  }
  {
    ...
    secretValue: substring(uniqueString('dbadmin', resourceGroup().id, configuration.name, location), 0, 8)
  }
  {
    secretName: 'airflow-db-connection'
    secretValue: 'postgresql://dbuser:${substring(uniqueString('dbuser', resourceGroup().id, configuration.name, location), 0, 8)}@airflow-cluster-rw.postgresql.svc.cluster.local:5432/airflow-db'
  }
  {
  ...
  }
  {
    ...
    secretValue: substring(uniqueString('airflow', resourceGroup().id, configuration.name, location), 0, 8)
  }
  {
    secretName: 'airflow-fernet-key'
    secretValue: substring(uniqueString('airflow-fernet', resourceGroup().id, configuration.name, location), 0, 8)
  }
  {
    secretName: 'airflow-webserver-key'
    secretValue: substring(uniqueString('airflow-webserver', resourceGroup().id, configuration.name, location), 0, 8)
  }
]
