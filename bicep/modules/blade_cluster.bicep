...
// B2s with 2 vCPUs and 4 GiB of memory. Minimal configuration for cost optimization.
@description('A Custom VM Size for System Pool (2x4 BURST:true)')
param vmSizeSystemPool string = 'Standard_B2s'

// B2s with 2 vCPUs and 4 GiB of memory. Minimal configuration for cost optimization.
@description('A Custom VM Size for User Pool (2x4 BURST:true)')
param vmSizeUserPool string = 'Standard_B2s'
...
module cluster './managed-cluster/main.bicep' = {
  ...
  params: {
    name: '${replace(bladeConfig.sectionName, '-', '')}${uniqueString(resourceGroup().id, bladeConfig.sectionName, location)}'
    location: location
